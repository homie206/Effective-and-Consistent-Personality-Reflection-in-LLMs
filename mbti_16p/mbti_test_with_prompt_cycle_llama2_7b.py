...
def get_model_examing_result(model_id):
    ...
    for mbti_item in prompt_template["mbti_prompt"]:
        ...
        output_file_name = f'/home/hmsun/Personality-in-LLMs/mbti_16p/llama2_7b_chat_result/{mbti_label_content}-induce-mbti-llama2-7b-chat-hf-output.txt'
        result_file_name = f'/home/hmsun/Personality-in-LLMs/mbti_16p/llama2_7b_chat_result/{mbti_label_content}-induce-mbti-llama2-7b-chat-hf-result.csv'
            ...
            for cycle in range(1, 101):
                ...
                for question_num, question in mbti_questions.items():
                    messages = [
                        ...
                        {"role": "user", "content":"You will be presented a statement to describe you. Please show the extent of how you agree the statement on a scale from 1 to 7, with 1 being agree and 7 being disagree. You can only reply a number from 1 to 7. Here is the statement: " + f"{question}"}
                    ]
                    terminators = [
...
def get_single_model_examing_result(model_id):
    pipeline = transformers.pipeline(
        "text-generation",
        model=model_id,
        model_kwargs={"torch_dtype": torch.bfloat16},
